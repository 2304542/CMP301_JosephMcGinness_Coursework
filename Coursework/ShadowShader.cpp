...
void ShadowShader::setShaderParameters(ID3D11DeviceContext* deviceContext, const XMMATRIX& worldMatrix, const XMMATRIX& viewMatrix, const XMMATRIX& projectionMatrix, ID3D11ShaderResourceView* texture, ID3D11ShaderResourceView* heightmapTexture, float maxHeight, bool isHeightmap, ShadowMap* shadowMaps[2], Light* lights[2])
{
	...
	heightmapPtr->maxHeight = maxHeight;
	heightmapPtr->isHeightmap = isHeightmap;
	heightmapPtr->padding = XMFLOAT2(0.0f, 0.0f);
	deviceContext->Unmap(heightmapBuffer, 0);
	deviceContext->VSSetConstantBuffers(1, 1, &heightmapBuffer);

	
	deviceContext->PSSetShaderResources(0, 1, &texture); // lighting affects textures 
	deviceContext->PSSetShaderResources(1, 1, &heightmapTexture); // lighting affects heightmap 
	ID3D11ShaderResourceView* shadowing[2] = { shadowMaps[0]->getDepthMapSRV(), shadowMaps[1]->getDepthMapSRV() }; // shadows 
	deviceContext->PSSetShaderResources(2, 2, shadowing);
	
	deviceContext->PSSetSamplers(0, 1, &sampleState);
	deviceContext->PSSetSamplers(1, 1, &sampleStateShadow);

	
	deviceContext->VSSetShaderResources(0, 1, &heightmapTexture); // makes it so heightmapTexture affects the vertices of a mesh
